...
// Notifikasi SKCK Baru (menggunakan kolom 'progres')
$query_skck_baru = "SELECT COUNT(*) AS jumlah FROM skck WHERE progres = 'baru'"; // Ganti 'baru' jika perlu
$result_skck_baru = mysqli_query($conn, $query_skck_baru);
$data_skck_baru = mysqli_fetch_assoc($result_skck_baru);
if ($data_skck_baru['jumlah'] > 0) {
    $notifikasi[] = [
        'tipe' => 'skck',
        'pesan' => 'Ada ' . $data_skck_baru['jumlah'] . ' berkas SKCK baru',
        'link' => '../on-petugas/skck.php' // Sesuaikan link
    ];
    $jumlah_notifikasi += $data_skck_baru['jumlah'];
}

// Notifikasi SIK Baru (menggunakan kolom 'progres')
$query_sik_baru = "SELECT COUNT(*) AS jumlah FROM sik WHERE progres = 'baru'"; // Ganti 'baru' jika perlu
$result_sik_baru = mysqli_query($conn, $query_sik_baru);
$data_sik_baru = mysqli_fetch_assoc($result_sik_baru);
if ($data_sik_baru['jumlah'] > 0) {
    $notifikasi[] = [
        'tipe' => 'sik',
        'pesan' => 'Ada ' . $data_sik_baru['jumlah'] . ' berkas SIK baru',
        'link' => '../on-petugas/sik.php' // Sesuaikan link
    ];
    $jumlah_notifikasi += $data_sik_baru['jumlah'];
}

// Notifikasi STTP Baru (menggunakan kolom 'progres')
$query_sttp_baru = "SELECT COUNT(*) AS jumlah FROM sttp WHERE progres = 'baru'"; // Ganti 'baru' jika perlu
$result_sttp_baru = mysqli_query($conn, $query_sttp_baru);
$data_sttp_baru = mysqli_fetch_assoc($result_sttp_baru);
if ($data_sttp_baru['jumlah'] > 0) {
    $notifikasi[] = [
        'tipe' => 'sttp',
        'pesan' => 'Ada ' . $data_sttp_baru['jumlah'] . ' berkas STTP baru',
        'link' => '../on-petugas/sttp.php' // Sesuaikan link
    ];
    $jumlah_notifikasi += $data_sttp_baru['jumlah'];
}
...
<div class="header-content-right">
    ...
    <div class="header-element position-relative">
        <a href="chat.php" class="header-link">
            <i class="fe fe-message-square header-link-icon"></i>
            <?php if ($jumlah_chat_baru > 0): ?>
                <span class="badge bg-danger header-icon-badge pulse pulse-danger"
                    id="chat-icon-badge"><?php echo $jumlah_chat_baru; ?></span>
            <?php endif; ?>
        </a>
    </div>
